...
def train_eval_dist(rank: int, world_size: int, trainset: Dataset, testset: Optional[Dataset],
                    model: Module, loss_fn: Module, optimizer: Union[DistSGD, Signum],
                    lr_decay_step: int = 30, lr_decay_rate: float = 0.1, batch_size: int = 64,
                    n_epochs: int = 10, task: str = "classification", save_loss: bool = True,
                    save_acc: bool = True, folder_name: str = "tmp", save_step: int = 10,
                    verbose: int = 1):
    ...
    optimizer : `src.optim.DistSGD` or `src.optim.Signum`
    ...
    if verbose in {1, 2} and rank == 0:  # show a progress bar for the server
        ...
        pbar_desc = f"Rank 0/{world_size-1}; LR {current_lr}; First epoch..."
        pbar = tqdm(range(n_epochs), desc=pbar_desc)
    ...
    for epoch in pbar:
        ...
        for x_train, y_train in trainloader:
            ...
            if task == "classification":
                ...
                train_acc_value = (y_pred == y_train.flatten()).sum().item()
        ...
        if verbose in {1, 2}:
            ...
            # Update progress bar and print summary if verbose
            if rank == 0:

                # Compute mean training scores
                mean_train_loss_epoch = all_scores[:, 0].mean().item()
                mean_train_acc_epoch = all_scores[:, 2].mean().item()

                # Update progress bar to show current mean scores
                pbar_desc = f"Epoch {epoch}; LR {current_lr};"
                pbar_desc += f" Mean training loss {mean_train_loss_epoch:.4f};"
                pbar_desc += f" Test loss {test_loss_epoch:.4f};"
                if task == "classification":
                    pbar_desc += f" Mean training acc. {mean_train_acc_epoch:.4f};"
                    pbar_desc += f" Test acc. {test_acc_epoch:.4f}"
                ...
                if verbose == 2:
                    ...
                    for worker in range(world_size):
                        ...
                        msg_worker += f" Training loss {all_scores[worker][0].item():.4f};"
                        msg_worker += f" Test loss {all_scores[worker][1].item():.4f};"
                        if task == "classification":
                            msg_worker += f" Training accuracy {all_scores[worker][2].item():.4f};"
                            msg_worker += f" Test accuracy {all_scores[worker][3].item():.4f}"
        ...
        if (epoch > 0 and (epoch + 1) % save_step == 0) or epoch == n_epochs-1:
...
def eval_dist(model: Module, testset: Dataset, loss_fn: Module, batch_size: int = 64,
    ...
    for x_test, y_test in testloader:
        ...
        if task == "classification":
            y_pred = torch.argmax(y_lsm_pred, dim=1)
            test_acc_value = (y_pred == y_test.flatten()).sum().item()
...
def save_metrics(rank: int, world_size: int, blind_size: int, byz_size: int,
                 metrics_scores: Dict[str, List[List[Union[int, float]]]], folder_name: str,
                 verbose: int = 1):
